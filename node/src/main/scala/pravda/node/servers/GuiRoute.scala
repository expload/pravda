                    ...
                    access =>
                      val eventuallyErrorOrTransaction =
                        for {
                          ...
                          PravdaAssembler.assemble(hackCode) map { data =>
                      ...
                      val eventuallyErrorOrTransaction =
                        for {
                          ...
                          val hackCode = code
                            ...
                            .replace("\\\"", "\"")
                            .replace("\\\\", "\\") // FIXME HACK CODE
                          PravdaAssembler.assemble(hackCode) flatMap { data =>
                            PravdaAssembler.assemble(s"new x${byteUtils.byteString2hex(data)} pcreate") map { data =>
                              val unsignedTx = UnsignedTransaction(Address.fromHex(address),
                                                                   TransactionData @@ data,
                                                                   wattLimit.toLong,
                                                                   NativeCoin.amount(wattPrice),
