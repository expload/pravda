      ...
      parsedAbi.map { abis =>
        ...
        run(Translator.translateActualContract(opcodes, abis), precondition) ==>
          Right(
            expectations(
              283L,
              stack = ArrayBuffer(BigInt(scala.BigInt(4)), BigInt(scala.BigInt(4)), BigInt(scala.BigInt(4)))))

        run(Translator.translateActualContract(opcodes, abis), precondition.copy(stack = Seq(Utf8("get")))) ==>
          Right(
            expectations(
              274L,
              stack = ArrayBuffer(BigInt(scala.BigInt(1)), BigInt(scala.BigInt(1)), BigInt(scala.BigInt(1)))))

        run(Translator.translateActualContract(opcodes, abis), precondition.copy(stack = Seq(Utf8("gegdft")))) ==>
          Right(
            expectations(218L, stack = ArrayBuffer(Utf8("gegdft")), error = Some(UserError("incorrect function name")))
          )
        ...
        run(Translator.translateActualContract(opcodes, abis), precondition.copy(stack = Seq(Utf8("set0")))) ==>
          Right(
            expectations(
              265L,
              stack = ArrayBuffer(BigInt(scala.BigInt(3)), BigInt(scala.BigInt(3)), BigInt(scala.BigInt(3)))))
      }
