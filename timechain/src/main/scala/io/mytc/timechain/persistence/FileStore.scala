...
import io.mytc.timechain.launcher
...
object FileStore {
  private lazy val organizationInfoFile = new File(launcher.timeChainConfig.dataDirectory, "info-organization.json")
  private lazy val nodeSettingsFile = new File(launcher.timeChainConfig.dataDirectory, "node-settings.json")
  ...
  def readPaymentWalletAsync(): Future[TimechainConfig.PaymentWallet] = {
    Future.successful(launcher.timeChainConfig.paymentWallet)
  }
