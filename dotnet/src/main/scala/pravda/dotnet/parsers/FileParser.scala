...
object FileParser {
  ...
  def parseFile(file: String): Either[String, (Pe, CilData, Seq[Method], Map[Long, Signatures.Signature])] = {
    val fileBytes = Files.readAllBytes(Paths.get(this.getClass.getResource(s"/$file").getPath))
    parsePe(fileBytes)
  }

  def parsePe(bytes: Array[Byte]): Either[String, (Pe, CilData, Seq[Method], Map[Long, Signatures.Signature])] = {
    val peV = PE.parseInfo(Bytes(bytes))

    for {
      pe <- peV
      cilData <- CIL.fromPeData(pe.peData)
      methods <- pe.methods.map(Method.parse(pe.peData, _)).toList.sequence
